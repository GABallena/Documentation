...
\dirtree{%
	.1 /project\_root/.
	.2 *path/to/cleaned\_reads/* (Prerequisite).
	.2 path/to/merged\_reads/.
	.2 *path/to/CARD\_db/* (Prerequisite - Database).
	.2 path/to/output/.
	...
	.2 *path/to/PFAM\_db/* (Prerequisite - Database).
	.2 *path/to/TnpPred\_db/* (Prerequisite - Database).
...
\dirtree{%
	...
	.2 configs/.
	.3 config.yaml.
	.2 **path/to/raw\_reads\_dir/** \textit{(Prerequisite - Raw Data)}.
	.3 sample1\_R1.fastq.gz.
	.3 sample1\_R2.fastq.gz.
	.3 sample2\_R1.fastq.gz.
	.3 sample2\_R2.fastq.gz.
	.2 **path/to/trimmed\_reads\_dir/** \textit{(Prerequisite - Trimmed Data)}.
...
\dirtree{%
	.1 /project\_root/.
	.2 raw\_reads/ \textit{(permanent)} **(prerequisite)**.
	.3 sample1\_R1.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample1\_R2.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample2\_R1.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample2\_R2.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.2 output\_dir/.
	.3 fastp\_output/ \textit{(temporary)} \textit{(to be deleted)}.
	.4 iteration\_1/.
	.5 sample1\_R1\_fastp\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_fastp\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample2\_R1\_fastp\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample2\_R2\_fastp\_trimmed.fastq.gz \textit{(temporary)}.
	.4 iteration\_2/.
	.5 (same as iteration\_1 but with iteration\_2 files) \textit{(temporary)}.
	.4 iteration\_3/.
	.5 (same as iteration\_1 but with iteration\_3 files) \textit{(temporary)}.
	.3 trimmomatic\_output/ \textit{(temporary)} \textit{(to be deleted)}.
	.4 iteration\_1/.
	.5 sample1\_R1\_paired.fastq.gz \textit{(temporary)}.
	.5 sample1\_R1\_unpaired.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_paired.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_unpaired.fastq.gz \textit{(temporary)}.
	.5 (same for sample2) \textit{(temporary)}.
	.4 iteration\_2/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.4 iteration\_3/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.3 cutadapt\_output/ \textit{(temporary)} \textit{(to be deleted)}.
	.4 iteration\_1/.
	.5 sample1\_R1\_cutadapt\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_cutadapt\_trimmed.fastq.gz \textit{(temporary)}.
	.4 iteration\_2/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.4 iteration\_3/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.3 bbduk\_output/ \textit{(temporary)} \textit{(to be deleted)}.
	.4 iteration\_1/.
	.5 sample1\_R1\_bbduk\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_bbduk\_trimmed.fastq.gz \textit{(temporary)}.
	.4 iteration\_2/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.4 iteration\_3/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.3 sickle\_output/ \textit{(temporary)} \textit{(to be deleted)}.
	.4 iteration\_1/.
	.5 sample1\_R1\_sickle\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample1\_R2\_sickle\_trimmed.fastq.gz \textit{(temporary)}.
	.5 sample1\_singles\_sickle\_trimmed.fastq.gz \textit{(temporary)}.
	.4 iteration\_2/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.4 iteration\_3/.
	.5 (same structure as iteration\_1) \textit{(temporary)}.
	.2 logs/ \textit{(permanent)} **(prerequisite)**.
	.3 fastp\_params.tsv \textit{(permanent)} **(prerequisite)**.
	.3 trimmomatic\_params.tsv \textit{(permanent)} **(prerequisite)**.
	.3 cutadapt\_params.tsv \textit{(permanent)} **(prerequisite)**.
	.3 bbduk\_params.tsv \textit{(permanent)} **(prerequisite)**.
	.3 sickle\_params.tsv \textit{(permanent)} **(prerequisite)**.
}
...
\dirtree{%
	.1 /project\_root/.
	.2 configs/.
	.3 config.yaml **(prerequisite)**.
	.2 path/to/raw\_reads\_dir/ \textit{(permanent)} **(prerequisite)**.
	.3 sample1\_R1.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample1\_R2.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample2\_R1.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.3 sample2\_R2.fastq.gz \textit{(permanent)} **(prerequisite)**.
	.2 path/to/temp\_bootstrap\_dir/ \textit{(temporary)} \textit{(to be deleted after pipeline resolves)}.
	.3 sample1/ \textit{(temporary)}.
	.4 rep\_1\_R1.fastq.gz \textit{(temporary)}.
	.4 rep\_1\_R2.fastq.gz \textit{(temporary)}.
	.4 rep\_2\_R1.fastq.gz \textit{(temporary)}.
	.4 rep\_2\_R2.fastq.gz \textit{(temporary)}.
	.4 total\_reads.txt \textit{(temporary)}.
	.3 sample2/ \textit{(temporary)}.
	.4 rep\_1\_R1.fastq.gz \textit{(temporary)}.
	.4 rep\_1\_R2.fastq.gz \textit{(temporary)}.
	.4 rep\_2\_R1.fastq.gz \textit{(temporary)}.
	.4 rep\_2\_R2.fastq.gz \textit{(temporary)}.
	.4 total\_reads.txt \textit{(temporary)}.
	.3 rep\_1/ \textit{(temporary)}.
	.4 diversity\_matrices\_sample1\_rep\_1.tsv \textit{(temporary)} \textit{(moved to diversity\_results/)}.
	.4 diversity\_matrices\_sample2\_rep\_1.tsv \textit{(temporary)} \textit{(moved to diversity\_results/)}.
	.3 rep\_2/ \textit{(temporary)}.
	.4 diversity\_matrices\_sample1\_rep\_2.tsv \textit{(temporary)} \textit{(moved to diversity\_results/)}.
	.4 diversity\_matrices\_sample2\_rep\_2.tsv \textit{(temporary)} \textit{(moved to diversity\_results/)}.
	.2 logs/ \textit{(permanent)} **(prerequisite)**.
	.3 metagenomics\_pipeline\_sample1\_rep\_1.log \textit{(permanent)} **(prerequisite)**.
	.3 metagenomics\_pipeline\_sample1\_rep\_2.log \textit{(permanent)} **(prerequisite)**.
	.3 metagenomics\_pipeline\_sample2\_rep\_1.log \textit{(permanent)} **(prerequisite)**.
	.3 metagenomics\_pipeline\_sample2\_rep\_2.log \textit{(permanent)} **(prerequisite)**.
	.2 diversity\_results/ \textit{(permanent)} \textit{(contains moved files)} **(prerequisite)**.
	.3 diversity\_rep\_1.tsv \textit{(moved from temp\_bootstrap\_dir)}.
	.3 diversity\_rep\_2.tsv \textit{(moved from temp\_bootstrap\_dir)}.
	.2 aggregated\_results/ \textit{(permanent)} **(prerequisite)**.
	.3 diversity\_aggregate.tsv \textit{(permanent)}.
}
...
\dirtree{%
	.1 /project\_root/.
	.2 trimmed\_reads/ \textit{(prerequisite)}.
	.3 site1\_R1\_paired.fastq.gz \textit{(permanent)}.
	.3 site1\_R2\_paired.fastq.gz \textit{(permanent)}.
	.3 site2\_R1\_paired.fastq.gz \textit{(permanent)}.
	.3 site2\_R2\_paired.fastq.gz \textit{(permanent)}.
	.2 fastuniq/ \textit{(temporary)} \textit{(to be deleted)}.
	.3 site1\_R1\_uniq.fastq \textit{(temporary)}.
	.3 site1\_R2\_uniq.fastq \textit{(temporary)}.
	.3 site2\_R1\_uniq.fastq \textit{(temporary)}.
	.3 site2\_R2\_uniq.fastq \textit{(temporary)}.
	.2 fasta/ \textit{(permanent)}.
	.3 site1\_R1\_clean.fasta \textit{(permanent)}.
	.3 site1\_R2\_clean.fasta \textit{(permanent)}.
	.3 site2\_R1\_clean.fasta \textit{(permanent)}.
	.3 site2\_R2\_clean.fasta \textit{(permanent)}.
	.2 megahit\_output/ \textit{(permanent)}.
	.3 site1\_assembly/.
	.4 site1\_filtered\_contigs.fa \textit{(permanent)}.
	.3 site2\_assembly/.
	.4 site2\_filtered\_contigs.fa \textit{(permanent)}.
	.2 cdhit\_contigs/ \textit{(permanent)}.
	.3 site1\_cdhit\_contigs.fasta \textit{(permanent)}.
	.3 site2\_cdhit\_contigs.fasta \textit{(permanent)}.
	.2 CLEAN\_READS/ \textit{(permanent)} \textit{(moved)}.
	.3 site1/.
	.4 site1\_1.fastq \textit{(moved from fastuniq)}.
	.4 site1\_2.fastq \textit{(moved from fastuniq)}.
	.3 site2/.
	.4 site2\_1.fastq \textit{(moved from fastuniq)}.
	.4 site2\_2.fastq \textit{(moved from fastuniq)}.
	.2 binning\_output/ \textit{(permanent)}.
	.3 site1\_binning\_output/.
	.4 concoct\_bins.scaffolds2bin.tsv \textit{(permanent)}.
	.4 metabat2\_bins.scaffolds2bin.tsv \textit{(permanent)}.
	.4 maxbin2\_bins.scaffolds2bin.tsv \textit{(permanent)}.
	.3 site2\_binning\_output/.
	.4 (same as site1) \textit{(permanent)}.
	.2 dastool\_output/ \textit{(permanent)}.
	.3 DAS\_Tool\_bins\_site1 \textit{(permanent)}.
	.3 DAS\_Tool\_bins\_site2 \textit{(permanent)}.
	.2 reassembly\_output/ \textit{(permanent)}.
	.3 site1\_reassembly\_output/.
	.4 assembly.fasta \textit{(permanent)}.
	.3 site2\_reassembly\_output/.
	.4 assembly.fasta \textit{(permanent)}.
	.2 magpurify\_output/ \textit{(permanent)}.
	.3 site1\_cleaned.fasta \textit{(permanent)}.
	.3 site2\_cleaned.fasta \textit{(permanent)}.
	.2 metaquast\_output/ \textit{(permanent)}.
	.3 site1\_metaquast\_output/.
	.4 (MetaQUAST results) \textit{(permanent)}.
	.3 site2\_metaquast\_output/.
	.4 (MetaQUAST results) \textit{(permanent)}.
	.2 dereplication\_output/ \textit{(permanent)}.
	.3 dereplicated\_genomes/.
	.4 dereplicated\_genomes.fasta \textit{(permanent)}.
	.4 checkm2\_output/ \textit{(permanent)}.
	.5 quality\_report.tsv \textit{(permanent)}.
	.2 logs/ \textit{(permanent)}.
	.3 run\_megahit\_site1.log \textit{(permanent)}.
	.3 run\_cdhit\_site1.log \textit{(permanent)}.
	.3 (logs for each step) \textit{(permanent)}.
}
...
\dirtree{%
	.1 /project\_root/.
	.2 trimmed\_reads/ **\textit{(Prerequisite - Trimmed reads directory)}**.
	.3 sample1\_R1\_paired.fastq.gz.
	.3 sample1\_R2\_paired.fastq.gz.
	.3 sample2\_R1\_paired.fastq.gz.
	.3 sample2\_R2\_paired.fastq.gz.
	.2 kraken\_db/ **\textit{(Prerequisite - Kraken2 database directory)}**.
	.3 database files (.k2d, etc.).
	.2 kraken\_output/ \textit{(Output directory for Kraken2 reports)}.
	.3 sample1.k2report.
	.3 sample1.kraken2.
	.3 sample2.k2report.
	.3 sample2.kraken2.
	.2 bracken\_output/ \textit{(Output directory for Bracken reports)}.
	.3 sample1.bracken.
	.3 sample1.breport.
	.3 sample2.bracken.
	.3 sample2.breport.
...
\dirtree{%
	.1 /project\_root/.
	.2 **raw\_reads/** \textit{(Prerequisite - Raw Data)}.
	.3 sample1.fastq.gz.
	.3 sample2.fastq.gz.
	.3 \dots.
	.2 **trimmed\_reads/** \textit{(Prerequisite - Trimmed Data)}.
