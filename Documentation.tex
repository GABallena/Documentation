...
\textbf{Technical Notes}: The \texttt{Python} script will measure standard contig quality metrics e.g. N50, L50. \\
\textbf{Rationale}:
\textbf{Note}:
...
\begin{enumerate}
	...
	\item \textbf{Gradual Drops}: Overlapping reads, repetitive or duplicate regions, partial HGT, sequence heterogeneity, or coverage differences due to a mixed population. Repeats and duplications can be filtered out using tools like \texttt{RepeatMasker} or \texttt{BLAST} (to be added to the script).
